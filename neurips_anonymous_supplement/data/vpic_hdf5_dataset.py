...
def _beta_from_filename(path: Path) -> float:
    ...
    processed_vpic_beta0.1.h5 -> 0.1
    """
    m = re.search(r"processed_vpic_beta([0-9]+(?:\.[0-9]+)?)\.h5$", path.name)
    if m is None:
...
def find_h5_files(
    ...
    files = sorted(
        h5_dir.glob("processed_vpic_beta*.h5"),
    ...
    if len(files) == 0:
        raise FileNotFoundError(
            "The processed VPIC corpus used in the paper is not distributed with this anonymous submission. "
            f"No matching HDF5 files were found in {h5_dir}. Place the files in data/hdf5 or pass --h5-dir / H5_DIR."
        )
...
class VPICWindowDataset(Dataset):
    ...
    def __init__(
        ...
        ----------
        h5_dir:
            Directory containing processed_vpic_beta*.h5 files.
...
if __name__ == "__main__":
    h5_dir = os.environ.get("H5_DIR", "data/hdf5")
